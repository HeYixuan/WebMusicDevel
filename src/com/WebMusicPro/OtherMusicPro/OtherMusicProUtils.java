...
/**
* <p>创建时间：2019年1月30日 下午8:05:31
* <p>项目名称：WebMusic
* 
* <p>类说明：OtherMusicPro工具集
*
* @version 1.0
* @since JDK 1.8
* 文件名称：OtherMusicProUtils.java
* */
public class OtherMusicProUtils extends Tools{
	
	/**<p>获取歌曲真实下载链接<p>需要传入一个参数: apiPhp<p>网站的链接*/
	public String getDownload(String apiPhp)
	{
		return getByJson(getPagesource(LinkList.MusicProLink+apiPhp), "url").replaceAll("\\\\", "");
	}
	
	/**<p>存储酷我音乐歌单信息<p>需要传入两个参数: page , list<WebMusicProInfo> <p>歌单页面内容,带WebMusicProInfo的list */
	...
	/**<p>存储网易云音乐歌单信息<p>需要传入两个参数: page , list<WebMusicProInfo> <p>歌单页面内容,带WebMusicProInfo的list */
	...
	/**<p>存储虾米音乐歌单信息<p>需要传入两个参数: page , list<WebMusicProInfo> <p>歌单页面内容,带WebMusicProInfo的list */
	...
	/**<p>存储酷苟音乐歌单信息<p>需要传入两个参数: page , list<WebMusicProInfo> <p>歌单页面内容,带WebMusicProInfo的list */
	...
	/**<p>存放酷我音乐歌曲各个音质直链<p>需要传入两个参数: s , w <p>返回的歌曲信息json列表 ，WebMusicProInfo实例对象 */
	...
	/**<p>存放网易云音乐歌曲各个音质直链<p>需要传入两个参数: s , w <p>返回的歌曲信息json列表 ，WebMusicProInfo实例对象 */
	...
	/**<p>存放虾米音乐歌曲各个音质直链<p>需要传入两个参数: s , w <p>返回的歌曲信息json列表 ，WebMusicProInfo实例对象 */
	...
	/**<p>存放酷狗音乐歌曲各个音质直链<p>需要传入两个参数: s , w <p>返回的歌曲信息json列表 ，WebMusicProInfo实例对象 */
	...
	/**<p>存放千千音乐(百度音乐)歌曲各个音质直链<p>需要传入两个参数: s , w <p>返回的歌曲信息json列表 ，WebMusicProInfo实例对象 */
	...
	/**<p>返回一个带有WebMusicProInfo的List<p>需要传入四个参数: search_name , maxpage , music_num , type <p>搜索的内容，最大页面数量(只能是整数型) , 一页歌曲的数量(只能是数字，而且最低只能是11) , 音乐类型*/
	...
	/**<p>返回一个带有WebMusicProInfo的List<p>需要传入三个参数: search_name , music_num , type <p>搜索的内容, 一页歌曲的数量(只能是数字，而且最低只能是11) , 音乐类型*/
	...
	/**<p>返回一个带有WebMusicProInfo的List<p>需要传入两个参数: link , type <p>歌单链接 , 音乐类型*/
	...
	/**<p>返回一个带有WebMusicProInfo的List<p>需要传入五个参数: search_name , startPage , endPage , music_num , type <p>搜索的内容  , 页面开始的位置 , 页面结束的位置, 一页歌曲的数量(只能是数字，而且最低只能是11) , 音乐类型*/
	public List<WebMusicProInfo> getInfoList(String search_name , int startPage , int endPage , int music_num , String type)
	{
		...
		if(startPage > 0)
		{
			if(endPage >startPage)
			{
				...
				{
					String str = getPagesource(LinkList.MusicProLink+LinkList.MusicProSearchTypeLink+type+LinkList.MusicProSearchLink+URLEncode(search_name)+LinkList.MusicProPageLink+i+LinkList.MusicProMusicNumLink+music_num);
					Matcher mid = Pattern.compile("\"songmid\":(.+?\")(.+?\")").matcher(str);
					while(mid.find())
					{
			...
			{
				String str = getPagesource(LinkList.MusicProLink+LinkList.MusicProSearchTypeLink+type+LinkList.MusicProSearchLink+URLEncode(search_name)+LinkList.MusicProPageLink+startPage+LinkList.MusicProMusicNumLink+music_num);
				Matcher mid = Pattern.compile("\"songmid\":(.+?\")(.+?\")").matcher(str);
				while(mid.find())
				{
					l.add(Info(mid.group().replaceAll("songmid|songname|:|,|\"", ""),type));
		...
		{
			String str = getPagesource(LinkList.MusicProLink+LinkList.MusicProSearchTypeLink+type+LinkList.MusicProSearchLink+URLEncode(search_name)+LinkList.MusicProPageLink+1+LinkList.MusicProMusicNumLink+music_num);
			Matcher mid = Pattern.compile("\"songmid\":(.+?\")(.+?\")").matcher(str);
			while(mid.find())
			{
				l.add(Info(mid.group().replaceAll("songmid|songname|:|,|\"", ""),type));
