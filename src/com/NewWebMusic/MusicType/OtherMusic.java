...
import com.NewWebMusic.util.MusicTools;
import com.NewWebMusic.util.NewWebMusicTools;
...
public class OtherMusic implements WebMusic {
	...
	public void getInfo(String search_name) {
		new NewWebMusicTools().printList(new MusicTools().getInfoList(search_name, checkType(getType())));
	}

	@Override
	public void getInfo(String search_name, int maxpage) {
		new NewWebMusicTools().printList(new MusicTools().getInfoList(search_name, checkType(getType()), maxpage));
	}

	@Override
	public String getInfoToString(String search_name) {
		return new NewWebMusicTools().printListString(new MusicTools().getInfoList(search_name, checkType(getType())));
	}

	@Override
	public String getInfoToString(String search_name, int maxpage) {
		return new NewWebMusicTools()
				.printListString(new MusicTools().getInfoList(search_name, checkType(getType()), maxpage));
	}

	@Override
	public String[] getInfoToStringArray(String search_name) {
		return new NewWebMusicTools()
				.printListStringArray(new MusicTools().getInfoList(search_name, checkType(getType())));
	}

	@Override
	public String[] getInfoToStringArray(String search_name, int maxpage) {
		return new NewWebMusicTools()
				.printListStringArray(new MusicTools().getInfoList(search_name, checkType(getType()), maxpage));
	}
	...
	public WebMusicInfo[] getInfoToWebMusicInfo(String search_name, int maxpage) {
		return new NewWebMusicTools()
				.printListInfo(new MusicTools().getInfoList(search_name, checkType(getType()), maxpage));
	}

	@Override
	public void getInfo(String search_name, int startPage, int endPage) {
		new NewWebMusicTools()
				.printList(new MusicTools().getInfoList(search_name, startPage, endPage, checkType(getType())));
	}

	@Override
	public String getInfoToString(String search_name, int startPage, int endPage) {
		return new NewWebMusicTools()
				.printListString(new MusicTools().getInfoList(search_name, startPage, endPage, checkType(getType())));
	}

	@Override
	public String[] getInfoToStringArray(String search_name, int startPage, int endPage) {
		return new NewWebMusicTools().printListStringArray(
				new MusicTools().getInfoList(search_name, startPage, endPage, checkType(getType())));
	}

	@Override
	public WebMusicInfo[] getInfoToWebMusicInfo(String search_name, int startPage, int endPage) {
		return new NewWebMusicTools()
				.printListInfo(new MusicTools().getInfoList(search_name, startPage, endPage, checkType(getType())));
	}
		...
		if (startPage > 0) {
			if (endPage > startPage) {
				for (WebMusicInfo w : getInfoToWebMusicInfo(search_name, startPage, endPage)) {
					new NewWebMusicTools().MultiThreadDownload(checkSave(getSave()), w.getFileName(),
							w.getDownloadLink());
				}
			} else {
			...
			for (WebMusicInfo w : getInfoToWebMusicInfo(search_name, 1, 1)) {
				new NewWebMusicTools().MultiThreadDownload(checkSave(getSave()), w.getFileName(), w.getDownloadLink());
			}
		}
	}
